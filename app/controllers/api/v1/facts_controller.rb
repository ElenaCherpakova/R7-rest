class Api::V1::FactsController < ApplicationController
  ...
  def index
    ...
    render json: @member.facts # NOTE: that because facts route is nested inside members
  ...
  def create
    ...
    if @fact.save
      ...
      render json: { error: "The fact entry could not be created #{@fact.errors.full_message.to_sentence}" },
             status: 400
    end
  end
  ...
  def update
    ...
    if @fact.update(fact_params)
      ...
      render json: { error: 'Could not update facts' }, status: 500
    end
  end
